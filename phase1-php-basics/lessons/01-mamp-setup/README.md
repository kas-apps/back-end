...
2. 解凍されたダウンローダーをダブルクリック
    - 「Download」をクリック

3. インストーラーが起動するので、指示に従う
    - 「続ける」をクリック
    - 「同意する」をクリック
    - 「インストール」をクリック
...
![MAMPスタートページ](mamp-start-page.png)
...
- MAMPウィンドウの「Preferences」から「Server」タブの「Opne in Finder」をクリック
...
3. 新規ファイルを作成し、ファイル名を `hello.php` として保存
    - 保存場所：`htdocs` フォルダの中
...
1. 新しいファイルを作成し、ファイル名を `info.php` として保存
   - 保存場所：`htdocs` フォルダの中
...
1. 「Preferences」→「Server」タブ
2. 「Choose...」ボタンで好きなフォルダを選択
3. 「OK」をクリック
